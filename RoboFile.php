...
class RoboFile extends Tasks implements LoggerAwareInterface {
  ...
  public function test($options = [
    ...
    $task
      ...
      ->exec("$bin/phpunit {$this->bltRoot}/tests/phpunit --group blt-project -c {$this->bltRoot}/tests/phpunit/phpunit.xml")
      ->exec("$bin/phpunit {$this->bltRoot}/tests/phpunit --group blt-project -c {$this->bltRoot}/tests/phpunit/phpunit.xml")
      // Run 'blt' phpunit tests, excluding deploy-push tests.
      ->exec("$bin/phpunit {$this->bltRoot}/tests/phpunit --group blt --exclude-group deploy-push")
  ...
  public function bltRelease(
    ...
    $this->assertBranchExistsUpstream($current_branch);
    $this->checkDirty();
   ...
   */
  protected function assertBranchExistsUpstream($current_branch, $remote = 'origin') {
